...
lidar1_point_cloud_path = ".\\find_position\\PointClouds1\\" + data_index + ".pcd"
lidar2_point_cloud_path = ".\\find_position\\PointClouds2\\" + data_index + ".pcd"
...
T = world_points_mass_center.reshape((3,1)) - np.dot(R, lidar_points_mass_center.reshape((3,1)))
print(f"R_1: \n{R}")
print(f"T_1: {T}")
print(f"C_w: {C_w}")
print(world_points)
...
# print(R_2)
# print(VT, VT.T)
T_2 = world_points_mass_center.reshape((3,1)) - np.dot(R_2, lidar_points_mass_center.reshape((3,1)))
print(f"R_2: \n{R_2}")
print(f"T_2: {T_2}")
print(f"C_w: {C_w}")
print(world_points)
...
print("T: ")
print(np.dot(R_2.T, (T_2-T)))
print("i think it's make sense but is wrong")
print(np.dot(R_2.T, (T - T_2)))

R_ = np.dot(R_2.T, R) 
print(f"det(R): {np.linalg.det(R)}")
print(f"det(R_2): {np.linalg.det(R_2)}")
print(f"det(R_): {np.linalg.det(R_)}")

world_to_zed_R = findCamera.rmat
world_to_zed_T = findCamera.tvecs/1000


lidar2_to_zed_R = np.dot(world_to_zed_R, R_2)
lidar2_to_zed_T = np.dot(world_to_zed_R, T_2) + world_to_zed_T

print(f"lidar to zed R:\n{lidar2_to_zed_R}")
print(f"lidar to zed T:\n{lidar2_to_zed_T}")

zed_to_lidar2_R = np.dot(R_2.T, world_to_zed_R.T)
zed_to_lidar2_T = -1*np.dot(R_2.T, np.dot(world_to_zed_R.T, world_to_zed_T) + T_2)

print(f"zed to lidar2 R:\n{zed_to_lidar2_R}")
print(f"zed to lidar2 T:\n{zed_to_lidar2_T}")